...
import { Favorite, Params } from "../core/interfaces";
...
class FavouriteService extends ApiBridge<Favorite>{
  ...
  async addFavourites(body: any, token: string) {
    ...
    try {
      ...
      const bands = await Promise.all(data.bandsIds.map((id: string) =>
        bandService.getEntityById(id)
      ));
      const genres = await Promise.all(data.genresIds.map((id: string) =>
        genreService.getEntityById(id)
      ));
      const artists = await Promise.all(data.artistsIds.map((id: string) =>
        artistsService.getEntityById(id)
      ));
      const tracks = await Promise.all(data.tracksIds.map((id: string) =>
